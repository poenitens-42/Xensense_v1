...
# Utility: Create mask for object inpainting (  future work)
...
def main():
    ...
    while True:
        ...
        for det in smoke_dets:
            sx1 = int(det["x1"]*scale_x)
            sy1 = int(det["y1"]*scale_y)
            sx2 = int(det["x2"]*scale_x)
            sy2 = int(det["y2"]*scale_y)
        ...
        for track in tracks:
            ...
            if depth_map is not None:
                ...
                dist_normalized = depth_map[cy_clamped, cx_clamped]
                approx_distance_m = (1 - dist_normalized) * cfg.get("max_depth_m", 50)
            else:
            ...
            if hazard:
                ...
                play_beep(frequency=1000 if cls_name != "pothole" else 600, duration_ms=200)

            # Draw normal box if not hazard
            else:
        ...
        # Depth estimation every depth_skip frames
